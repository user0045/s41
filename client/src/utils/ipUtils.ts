...
export const checkRecentAdRequest = async (userIP: string): Promise<boolean> => {
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000).toISOString();
  
  try {
    const { data, error } = await fetch('/api/check-recent-ad-request', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userIP, since: oneHourAgo })
    }).then(res => res.json());
    
    if (error) throw error;
    return data.hasRecentRequest;
  } catch (error) {
    console.error('Error checking recent ad request:', error);
    return false;
  }
};
